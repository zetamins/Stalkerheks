...
function formHTML(cfg){const p=cfg.portal||{};const s=cfg.services||{}
  return'<div class="form-row"><div class="form-group"><label>Profile Name *</label><input id="f-name" value="'+esc(cfg.name||'')+'" placeholder="my-profile"></div><div class="form-group"><label>Model</label><input id="f-model" value="'+esc(p.model||'MAG254')+'" placeholder="MAG254"></div></div>'+
    ...
    '<div class="form-group"><label>CDN MAC (optional)</label><input id="f-cdn-mac" value="'+esc(p.cdn_mac||'')+'" placeholder="leave empty to use the MAC above"><div class="hint">Overrides mac= in stream play URLs. Leave empty for normal playback. Only set a separate MAC if your provider needs it to bypass per-MAC 458 anti-sharing — a random/unregistered one can break live TV.</div></div>'+
    '<div class="form-group"><label>Device ID *</label><input id="f-device-id" value="'+esc(p.device_id||'')+'" placeholder="64-char hex"><div class="hint">device_id2 and signature derived from UID Secret if empty</div></div>'+
    '<div class="form-row"><div class="form-group"><label>Device ID2 (auto)</label><input id="f-device-id2" value="'+esc(p.device_id2||'')+'" placeholder="Auto-derived"></div><div class="form-group"><label>Signature (auto)</label><input id="f-signature" value="'+esc(p.signature||'')+'" placeholder="Auto-derived per handshake"></div></div>'+
    '<div class="form-group"><label>UID Secret</label><input id="f-uid-secret" value="'+esc(p.uid_secret||'')+'" placeholder="Auto-generated if empty"><div class="hint">root secret device_id2/signature are derived from — software stand-in for a real STB\'s TEE-held hardware key</div></div>'+
    ...
    '<div class="form-group"><label>Portal URL #2 (fallback)</label><input id="f-url2" value="'+esc(p.url2||'')+'" placeholder="optional, tried if Portal URL is unreachable"></div>'+
...
function readForm(){return{name:el('f-name'),portal:{model:el('f-model')||'MAG254',serial_number:el('f-sn'),device_id:el('f-device-id'),device_id2:el('f-device-id2'),signature:el('f-signature'),mac:el('f-mac'),cdn_mac:el('f-cdn-mac'),url:el('f-url'),url2:el('f-url2'),time_zone:el('f-tz')||'Europe/Vilnius',token:el('f-token'),uid_secret:el('f-uid-secret')},services:{proxy_bind:el('f-proxy-bind')||'0.0.0.0:8888',hls_bind:el('f-hls-bind')||'0.0.0.0:9999'}}}
