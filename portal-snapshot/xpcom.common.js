...
function common_xpcom(){
    ...
    this.load = function(params, var_args){
        ...
        try{
			...
			req.setRequestHeader("SN", this.serial_number);
    ...
    this.user_init = function(user_data){
        ...
        if (this.user['allowed_stb_types'].length && !this.profile['strict_stb_type_check'] && this.user['allowed_stb_types'].indexOf('aurahd') !== -1){
            var cut_type = this.type.indexOf('AuraHD') != -1 ? 'AuraHD' : this.type;
        }else{
            cut_type = this.type;
        }

        if (this.user['allowed_stb_types'].length && this.user['allowed_stb_types'].indexOf(cut_type.toLowerCase()) == -1 && !_GET['debug_key']){
